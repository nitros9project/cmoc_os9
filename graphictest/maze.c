...
void circle_draw_solid(unsigned int i, unsigned int j, unsigned int dir) {
     ...
     //        _cgfx_setdptr(outpath, tempx + (int)(XOFFSET * 1.5), tempy +
     //        YOFFSET);
...
void solve_maze() {
     ...
     /*
     Pixmap tile25;
     tile25 = XwCreateTile ( DefaultScreen( display ), WhitePixel(display,
     DefaultScreen(display)), BlackPixel(display, DefaultScreen(display)),
     Xw25_FOREGROUND );
     */
     /* do it */
     while (1) {
...
int main(int argc, char **argv) {
     ...
     // keys are mapped as per
     // https://nitros9.sourceforge.io/wiki/index.php/SS.KySns_Get_Status
     int keys;
