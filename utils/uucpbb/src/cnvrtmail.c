...
int last_user;
{
     ...
     answers = (char *)malloc (last_user * sizeof (char));
          ...
          if (strncmp (line, ">From ", 6) == 0
              || strncmp (line, "From ", 5) == 0)
            {
               ...
               sprintf (fname, "%s/m%s%02d%02d%02d%02d%02d%02d",
                        users[uindex].name,
                        date.year > 90  &&  date.year <= 99 ? "19" : "20",
...
unsigned userid;
{
     ...
     closedir (dirptr);
     qsort (ltrs, mailcount, sizeof (struct mbag),
            (int (*)(const void *, const void *)) strucmp);
