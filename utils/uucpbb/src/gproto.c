          ...
          if (tseq != sseq)
            {
               ...
               cksum = MAGIC - ((outp->C0 & 0xff) | ((unsigned)outp->C1 << 8));
               cksum ^= (outp->C & 0xFF);
               ...
               outp->C0 = (char)(cksum & 0xFF);
               outp->C1 = (char)((cksum >> 8) & 0xFF);

               outp->X = outp->K ^ outp->C0 ^ outp->C1 ^ outp->C;

               write(port, (char *) outp, size + 6);
...
register int code;
{
     ...
     outp.C0 = (char)(cksum & 0xFF);
     outp.C1 = (char)((cksum >> 8) & 0xFF);
     outp.X = (char)(outp.K ^ outp.C0 ^ outp.C1 ^ code);

     if (debuglvl > 7)
          dumpcode (">> ", &outp);

     write (port, (char *) &outp, 6);
     return 0;
}
...
int  *length;
{
     ...
     *data = '\0';
     ...
     if (cksum != (((unsigned)inpacket.C1 << 8) | (inpacket.C0 & 0xFF)))
          return (FALSE);
     else
       {
          if (*length)
