...
{
     if (x > 99)
          putch (p, (char)(x/100 + '0'));

     if (x > 9)
          putch (p, (char)(x%100/10 + '0'));
...
char *name;
{
     ...
     _os_getime (p);
     sprintf (curtime, "%02d%02d", p->hours, p->minutes);
...
char *buffer;
{
     ...
     for (;;)
       {
          _os_getime (&t);                              /* start/reset timer */

          if ((tdone = t.seconds + TIMEOUT) > 59)
               tdone -= 60;
...
int onoroff;
{
     ...
     _gs_opt (1, &s);
     s.sg_echo = (unsigned char) onoroff;
     _ss_opt (1, &s);
     return 0;
}
