...
char *argv[];
{
     ...
     if ((file = fopen (newsrcfile, "r")) != NULL)
     ...
     else
       {
          ...
          for (i = 0; i < ngroups; i++)
               if (findstr (groups[i].newsgroup, "junk") != NULL)
                    newsrc[i].sub = UNSUBSCRIBED;
               else if (findstr (groups[i].newsgroup, ".test") != NULL)
                    newsrc[i].sub = UNSUBSCRIBED;
               else
     ...
     while (seq > indx || curgroup == ngroups)
       {
          ...
          fflush (stdout);
          c = getresponse();
          ch = c = (char) tolower (c);      /* 'ch' is temp used only in switch() */
          ...
          if (header)
            {
               ...
               if (strncmp (lp, "Keywords: ", 10) == 0)
                    if (auto_rot && findstr (lp, "rot13") != NULL)
                         rot13 = TRUE;
...
int onoroff;
{
     ...
     _gs_opt (1, &stdinpath);
     stdinpath.sg_echo = (unsigned char) onoroff;             /* switch standard input echo */
...
int x, y, width, height;
{
     ...
     outstr[5] = outstr[14] = (char) width;
     outstr[6] = outstr[15] = (char) height;

     /* bottom window */
     outstr[3] = (char) x + 1;                    /* offset bottom window */
     outstr[4] = (char) y + 1;
