...
FILE *file;
{
     ...
     sprintf (envelop, "m%02d%02d%02d%02d%02d%02d%02d",
                       (tod.year >= 92) && (tod.year <= 99)  ? 19  : 20,
                       tod.year, tod.month, tod.day, tod.hours,
                       tod.minutes, tod.seconds);
...
flag islocal;
{
     ...
     *addr = *neighbor = '\0';
     ...
     while (((p = findstr (line, "From ")) != NULL)
            && ((n = p - line + 1) <= 2)  &&  (n > 0))
       {
          strcpy (line2, line);
               ...
               if (*line == 'F'  &&  !gotfrom)
                 {
                    if (strnucmp (line, "From: ", 6) == 0)
                      {
                         strncpy (frm, grabfrom (line), FRMSIZE);
                         frm[FRMSIZE - 1] = '\0';
                         gotfrom = TRUE;
                      }
                 }
                    ...
                    if (strnucmp (line, "Subject: ", 9) == 0
                         ||  strnucmp (line, "Subj: ", 6) == 0)
                      {
                         ...
                         strncpy (subj, getstring (line), SUBJSIZE);
                         subj[SUBJSIZE - 1] = '\0';
                         gotsubj = TRUE;
                      }
