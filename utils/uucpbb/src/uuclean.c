     ...
     while ((pdir = readdir (dir)) != NULL)
       {
          ...
          if (strncmp ((char *) fdbuf.fd_date, def_date, 5) <= 0)
            {
               if (got_data_file)
                    mail_owner();
...
struct os_time *datebuff;
{
     ...
     datebuff->year   = (unsigned char) utimeptr->tm_year;
#ifdef _OSK
     datebuff->month  = utimeptr->tm_mon + 1;     /* OSK is different!!! */
#else
# ifndef NEWCLIB                                 /* early Kreider CLIB.L    */
     datebuff->month  = utimeptr->tm_mon;      /* does it one way, newer  */
# else                                           /* versions (1990) does it */
     datebuff->month  = (unsigned char) utimeptr->tm_mon + 1;  /* like OSK...confused??   */
# endif                                          /* me, too --REB           */
#endif
     datebuff->day    = (unsigned char) utimeptr->tm_mday;
     datebuff->hours   = (unsigned char) utimeptr->tm_hour;
     datebuff->minutes = (unsigned char) utimeptr->tm_min;
     datebuff->seconds = (unsigned char) utimeptr->tm_sec;

     /* Setup a string with the date in it for possible printing.
        Added [MDG] */

     sprintf (timestr,"%02d/%02d/%02d %02d:%02d:%02d",
                      datebuff->month, datebuff->day, datebuff->year,
                      datebuff->hours, datebuff->minutes,
                      datebuff->seconds);
/*   return (datebuff); */
     return 0;
}
